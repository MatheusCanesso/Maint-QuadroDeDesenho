...
const buttonClear = document.querySelector(".button__toolClear");
...
const draw = (x, y) => {
  ctx.globalCompositeOperation = "source-over"; // documentação
  ctx.beginPath();
  ctx.arc(
    x - canvas.offsetLeft,
    y - canvas.offsetTop,
...
const erase = (x, y) => {
  ctx.globalCompositeOperation = "destination-out"; // documentação
  ctx.beginPath();
  ctx.arc(
    x - canvas.offsetLeft,
    y - canvas.offsetTop,
...
const selectSize = ({ target }) => {
  ...
  if (selectedTool) {
    ...
    if (size) {
      sizeButtons.forEach((tool) => tool.classList.remove("activeSize"));
      selectedTool.classList.add("activeSize");
      brushSize = size;
    }
  }
};
